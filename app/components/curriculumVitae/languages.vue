...
/** Only the prose entries need the full row; a certification line fits a half. */
const isWide = (item: LanguageEntry) => Boolean(item.usage || item.evidence || item.note)
// #endregion
</script>
<template>
  <div class="h-fit">
    ...
    <div v-if="languages.length" class="mb-12">
      ...
      <ul class="grid grid-cols-1 md:grid-cols-2 gap-6 list-none p-0" aria-labelledby="languages-heading">
        <li
          v-for="item in languages"
          :key="item.bcp47"
          class="glass-card p-6 hover:bg-dark-900/60 transition-colors duration-300"
          :class="{ 'md:col-span-2': isWide(item) }"
        >
          <div class="flex items-start gap-5">
            ...
            <div class="grow min-w-0">
              <div class="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-2 mb-4">
                <h3 class="font-bold text-xl text-slate-100 font-display flex items-baseline gap-2">
                  {{ item.name }}
                ...
                <!-- The level reads as text in every case: nothing here is
                     carried by colour alone. -->
                <span
                  v-if="item.level === 'native'"
                  class="text-xs font-bold px-2 py-1 rounded bg-dark-950/50 border border-primary-500/30 text-primary-400"
